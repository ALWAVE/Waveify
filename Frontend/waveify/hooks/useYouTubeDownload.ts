...
async function downloadAudio(params: DownloadParams) {
  ...
  try {
    const response = await fetch("https://localhost:7040/YouTube/download", {
      method: "POST",
    ...
    // ВАЖНО: получить заголовок
    const contentDisposition = response.headers.get("content-disposition");
    console.log("Content-Disposition header:", contentDisposition); // Проверяем, что фронт его видит!
