...
const ProfileButton = () => {
  ...
  // роль теперь строкой:
  const label = user?.roleName ?? user?.role ?? "Guest";
  ...
  const hasPremiumSubscription = !!user?.sub;

  return (
    <div className="relative z-50" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`no-drag relative group hover:scale-110 cursor-pointer text-[var(--text)] font-semibold px-4 py-2 rounded-full transition p-4 ${
          hasPremiumSubscription
            ? gradientMap[user?.subColor as keyof typeof gradientMap]
            : "bg-[var(--bgProfile)]"
        }`}
      >
      ...
      {isOpen && (
        <div className="absolute right-[-15px] mt-2 w-58 bg-[var(--bgProfile)] text-white rounded-lg shadow-lg z-50">
          <div className="flex items-centert justify-left pt-4 px-6 text-sm ">
            <div className="text-[var(--text)] relative group">
              {user?.userName}
              <Tooltip label={label} />
            </div>
          </div>
          ...
          <p className="px-6 pb-2 text-neutral-400 text-sm">
            {user?.subEndDate
              ? "Подписка до: " + new Date(user.subEndDate).toLocaleDateString()
              : ""}
          </p>
          ...
          {/* Проверка на модератора по строковой роли */}
