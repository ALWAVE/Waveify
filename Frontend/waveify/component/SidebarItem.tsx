...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  // Контент: текст слева, иконка справа
  const content = (
    <>
  ...
  return (
    <div className="relative group">
      {isButton ? (
        <button onClick={handleClick} className={baseClasses} aria-label={label || undefined}>
          {content}
        </button>
      ) : (
        <Link href={href} onClick={handleClick} className={baseClasses} aria-label={label || undefined}>
          {content}
        </Link>
      )}

      {!label && <Tooltip position={positionToolTipe} label={label || "Нет названия"} />}
    </div>
  );
};
